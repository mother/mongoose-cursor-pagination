...
module.exports = (schema, fieldTransformers = {}) => {
   ...
   const newExec = async function (_origExec, ...args) {
      ...
      if (hasNext) {
         ...
         nextCursor = generateCursorStr(nextCursorObj, fieldTransformers)
      }
   ...
   schema.query.paginate = function (start) {
      ...
      this.exec = newExec.bind(this, origExec)
      return this
   }
   ...
   schema.pre('find', function () {
      ...
      if (typeof query.__paginationPlugin === 'object') {
         ...
         if (typeof start !== 'undefined') {
            const cursorObj = parseCursorStr(start, fieldTransformers)
